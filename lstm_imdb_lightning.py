...
이 파일은 기존 코드의 torchtext.legacy 의존성을 제거한 버전입니다.
기존 코드의 문제점은 다음과 같습니다.
1. torchtext.legacy는 최신 torchtext에서 제거되어 ModuleNotFoundError가 발생합니다.
2. torchtext와 torch의 바이너리 버전이 맞지 않으면 libtorchtext.pyd 로딩 오류가 발생합니다.
3. NumPy 2.x와 일부 PyTorch 2.2.x 조합에서는 NumPy 초기화 경고가 발생할 수 있습니다.

따라서 이 코드는 다음 방식으로 다시 작성했습니다.
1. torchtext를 사용하지 않습니다.
2. Python 표준 라이브러리로 IMDB 데이터를 다운로드하고 압축을 풉니다.
3. 직접 단어 사전(vocabulary)을 만듭니다.
4. torch.utils.data.Dataset과 DataLoader를 사용합니다.
5. PyTorch Lightning으로 학습, 검증, 테스트 과정을 구성합니다.
